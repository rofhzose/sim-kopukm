...
export default function PrintKibBChecklistPage() {
  const [data, setData] = useState([]);

  useEffect(() => {
    // Ambil data dari localStorage
    const storedData = localStorage.getItem("printCeklisBData");
    if (storedData) {
  ...
  return (
    <div className="p-8 bg-white min-h-screen text-black">
      <div className="text-center mb-6">
        <h1 className="text-xl font-bold uppercase">LAPORAN CEKLIS ASET (STOCKTAKE) KIB B</h1>
      ...
      <table className="w-full border-collapse border border-black text-xs">
        <thead>
          <tr className="bg-slate-100">
            ...
            <th className="border border-black p-2 w-28">Merk / Type</th>
        ...
        <tbody>
          {data.length === 0 ? (
            <tr>
              <td colSpan="9" className="border border-black p-4 text-center italic">Tidak ada data ceklis KIB B.</td>
            </tr>
          ) : (
            data.map((item, index) => (
